...
/*   Updated: 2020/04/09 18:47:06 by hna              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_width_p(t_option *opt, char *s_pad, char *s_prec, char *temp)
{
	...
	if (opt->minus == 1)
	{
		...
		ft_memset(&s_pad[ft_strlen(s_prec)], \
					' ', opt->width - ft_strlen(s_prec));
	}
	else
	{
		if (temp[0] == '-')
			ft_strncpy(&s_pad[opt->width - opt->prec - 1], \
...
static int	check_prec(t_option *opt, char *pr, char *s_pad, char *tp)
{
	char	*tmp_prec;
	...
	if (tp[0] == '-')
	{
		ft_strncpy(&pr[opt->prec - ft_strlen(tp) + 1], &tp[1], ft_strlen(tp));
		tmp_prec = malloc(sizeof(char) * (ft_strlen(pr) + 2));
		tmp_prec[0] = '-';
		ft_strcpy(&tmp_prec[1], pr);
		tmp_prec[opt->prec + 1] = '\0';
		free(pr);
		pr = tmp_prec;
	}
	else
		ft_strncpy(&pr[opt->prec - ft_strlen(tp)], tp, ft_strlen(tp));
	if (opt->width > opt->prec)
	{
		if (check_width_p(opt, s_pad, pr, tp) == -1)
			return (-1);
	}
	else
